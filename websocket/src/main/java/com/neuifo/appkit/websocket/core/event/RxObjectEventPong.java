...
import com.neuifo.appkit.websocket.core.object.event.RxObjectEventWrongMessageFormat;
...
public class RxObjectEventPong extends RxObjectEventConn {
    ...
    public RxObjectEventPong(@NonNull ObjectWebSocketSender sender, @NonNull Object message) {
        super(sender);
     ...
     */
    @NonNull
    public static <T> Observable.Transformer<RxObjectEventPong, T> filterAndMap(@NonNull final Class<T> clazz) {
        return new Observable.Transformer<RxObjectEventPong, T>() {
            @Override
            public Observable<T> call(Observable<RxObjectEventPong> observable) {
                return observable
                        .filter(new Func1<RxObjectEventPong, Boolean>() {
                            @Override
                            public Boolean call(RxObjectEventPong o) {
                                return o != null && clazz.isInstance(o.message());
                            }
                        })
                        .map(new Func1<RxObjectEventPong, T>() {
                            @Override
                            public T call(RxObjectEventPong o) {
                                return o.message();
                            }
                        });
            }
