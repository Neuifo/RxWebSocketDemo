...
class DecisionMaker {
    ...
    public void handleActionAdd(Notification notification, String packageName, String tag, int id, String key, Context context, String src) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            try {
                if (notification.extras.getString("android.template", "").equals("android.app.Notification$InboxStyle") && notification.extras.containsKey("android.textLines")) {
                    ...
                    bigText = "";
                    if (textLines != null) for (int i = textLines.length - 1; i >= 0; i--) {
                ...
                if (notification.bigContentView != null) {
                    try {
                        ...
                        try {
                            ...
                            try {
                                final String
                                    fullContent = fullContent(notification, context, texts, text);
                                if (fullContent != null) text = fullContent;
        ...
        List<EventPending> eventPendings = new ArrayList<>();
            ...
            try {
                ...
                if (actions != null) {
                    ...
                    for (Notification.Action action : actions) {
                        ...
                        eventPending.setIcon(action.icon);
                        eventPending.setTitle(title);
                        eventPending.setPendingIntent(action.actionIntent);
                        eventPendings.add(eventPending);
                    }
                }
        ...
        @ColorInt int color = android.R.color.white;
        if (Build.VERSION.SDK_INT >= 21) {
            color = notification.color;
        } else if (Build.VERSION.SDK_INT >= 19) {
            color = notification.extras.getInt("android.color");
        }

        //intent.putExtra("key", key);

        NcEvent ncEvent = new NotificationHelper.Builder()
                .setPackageName(packageName)
                .setNcAction(NcAction.STAY)
                .setTitle(title)
                .setContent(text)
                .setBasePending(notification.contentIntent)
                .setTag(tag)
                .setIcon(BitmapFactory.decodeResource(context.getResources(), notification.icon))
                .setId(id)
                .setColor(color)
                .setEventPendingList(eventPendings).build();
        ...
        /*if (preferences != null && preferences.getBoolean("broadcast_notifications", false)) {
            ...
            context.sendBroadcast(broadcastIntent, "com.higgs.app.permission.NOTIFICATIONS");
        }*/



        if ("listener".equals(src)) {
            NotificationHelper.sendNotificaiton(context,ncEvent, OverlayService.class);
        } else {
            NotificationHelper.sendNotificaiton(context,ncEvent, OverlayServiceCommon.class);
        }
    ...
    private String fullContent(Notification notification, Context context, List<String> texts, String text) {
        ...
        ViewGroup
            localView = (ViewGroup) inflater.inflate(notification.bigContentView.getLayoutId(), null);
        ...
        for (View view : allChildren) {
            if (view instanceof TextView) {
                ...
                Mlog.d(logTag, mText);
                if (!mText.equals(texts.get(0)) && mText.length() > 1 && !mText.matches("(([0]?[1-9]|1[0-2])([:.][0-5]\\d)(\\ [AaPp][Mm]))|(([0|1]?\\d?|2[0-3])([:.][0-5]\\d))") && !view.getClass().getSimpleName().equals("Button")) {
                    ...
                    if (mText.startsWith(texts.get(0))) {
                        ...
                        if (mText.startsWith(":")) mText = mText.substring(1);
    ...
    public void handleActionRemove(String packageName, String tag, int id, Context applicationContext) {
        ...
        NcEvent event = new NotificationHelper.Builder().setNcAction(NcAction.REMOVE).setTag(tag).setId(id).setPackageName(packageName).build();

        NotificationHelper.sendNotificaiton(applicationContext, event, OverlayService.class);
    ...
    public static boolean isNight(SharedPreferences preferences) {
        ...
        if (start < end) if (time >= start && time <= end) return true;
        if (start > end) if (time >= start || time <= end) return true;
        return false;
    }
    ...
    private static List<String> getText(Notification notification) {
        ...
        try {
            ...
            @SuppressWarnings("unchecked") ArrayList<Parcelable> actions = (ArrayList<Parcelable>) field.get(contentView);
