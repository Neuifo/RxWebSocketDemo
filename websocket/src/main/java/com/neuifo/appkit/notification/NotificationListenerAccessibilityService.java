        ...
        if (accessibilityEvent.getEventType() == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
            try {
               ...
               /* // Do not Ignore ongoing stuff if show non-cancelable feature is selected
                ...
                decisionMaker.handleActionAdd(notification, accessibilityEvent.getPackageName().toString(), null, 0, null, getApplicationContext(), "accessibility");
    ...
    protected void onServiceConnected() {
        ...
        if (Build.VERSION.SDK_INT >= 18) {
            ...
            NcEvent ncEvent = new NotificationHelper.Builder()
                    ...
                    .setContent(isNotificationListenerEnabled() ? getString(R.string.intro_warning_both_services) : getString(R.string.accessibility_desc).substring(getString(R.string.accessibility_desc).lastIndexOf("\n") + 1))
                    .setBasePending(PendingIntent.getActivity(getApplicationContext(), 0, new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS), 0))
                    .setIcon(BitmapFactory.decodeResource(getResources(), R.drawable.ic_dismiss_white))
                    .build();
            NotificationHelper.sendNotificaiton(getApplicationContext(), ncEvent);
            stopSelf();
        }
    }
